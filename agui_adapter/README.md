...
| `HERMES_AGUI_MAX_CONCURRENT_RUNS` | `8` | Cap on concurrent runs (one OS thread each). Read once at process start; set it in the process environment, not in `~/.hermes/.env`, which loads too late to reach it. |
